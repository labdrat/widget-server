...
const postgres = {
    ...
    async tryConnect(reconnect = false) {
        if (this.client || !this.connect || !this.disconnect)
            return;
        try {
            await this.connect();
            if (this.interval) {
...
app.get('/', async (_request, response) => {
    ...
    if (postgres.client) {
        ...
        if (!welcome.rows.length) {
            count = 1;
            await ((_a = postgres.client) === null || _a === void 0 ? void 0 : _a.query('INSERT INTO "welcome" VALUES($1)', [count]));
        }
        else {
            count = welcome.rows[0].count + 1;
            await ((_b = postgres.client) === null || _b === void 0 ? void 0 : _b.query('UPDATE "welcome" SET "count" = $1', [count]));
        }
    }
